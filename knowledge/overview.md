...
`install.sh` 与 `install.ps1` **失败关闭**：打印尚未发布并 exit 1，不改项目。`releases.json` 使用 schema v2，`releases` 为空；正式发行需 Ed25519 签名、公钥、HTTPS 下载地址和部署侧 `CPM_TRUSTED_PUBLIC_KEY` 信任锚。
